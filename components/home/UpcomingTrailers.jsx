...
export default function UpcomingTrailers({ peliculas = [] }) {
  const scrollRef = useRef(null);
  const watchlist = useMovieStore((state) => state.watchlist);
  const favoritas = useMovieStore((state) => state.favorites);
  const toggleStoreWatchlist = useMovieStore((state) => state.toggleWatchlist);
  const toggleStoreFavorite = useMovieStore((state) => state.toggleFavorite);
  ...
  const selectedMovie = useMemo(() => {
    return peliculas.find((movie) => movie.id === selectedId) ?? peliculas[0] ?? null;
  }, [peliculas, selectedId]);

  if (!peliculas.length || !selectedMovie) return null;

  const selectedTrailerUrl = buildYoutubeEmbedUrl(selectedMovie.trailer_key);
  const selectedBackdropUrl = selectedMovie.backdrop_path
  ...
  return (
    <section className="space-y-4 text-white">
      ...
      <article className="grid gap-4 overflow-hidden rounded-sm border border-white/10 bg-zinc-950 lg:grid-cols-[minmax(0,1fr)_320px]">
        ...
        <div className="flex flex-col justify-between gap-4 p-4 lg:p-5">
          <div className="min-w-0">
            ...
            <p className="mt-2 text-xs font-black uppercase tracking-[0.16em] text-white/62">
              Estreno {formatReleaseDate(selectedMovie.release_date)}
            </p>
            <p className="mt-4 line-clamp-5 text-sm leading-6 text-white/76">
              {selectedMovie.overview || "Selecciona otro estreno del carrusel para cambiar el trailer principal."}
            </p>
          </div>

          <div className="flex flex-wrap gap-2">
            <Link
              href={`/peli/${selectedMovie.id}#trailer`}
              className="inline-flex h-9 items-center justify-center rounded-sm bg-white px-4 text-sm font-bold text-black transition hover:bg-white/90"
            >
              Ver ficha
            </Link>
            <a
            ...
            return (
              <article key={movie.id} className="w-[316px] shrink-0 sm:w-[320px]">
                <button
                  ...
                  className={`group relative block aspect-video w-full overflow-hidden rounded-sm bg-zinc-900 text-left ring-offset-2 ring-offset-black transition ${
                    selectedMovie.id === movie.id ? "ring-2 ring-amber-300" : "hover:ring-1 hover:ring-white/40"
                  }`}
