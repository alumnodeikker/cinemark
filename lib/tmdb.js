...
function isAllowedRelease(movie) {
  ...
  return (
    year != null &&
    year >= RELEASE_YEAR_START &&
    year <= RELEASE_YEAR_END &&
    releaseDate >= upcomingReleaseDateStart() &&
    releaseDate <= RELEASE_DATE_END &&
...
function upcomingReleaseDateStart() {
  const today = todayIsoDate();
  return today > RELEASE_DATE_START ? today : RELEASE_DATE_START;
}
...
export async function getPopularMovies() {
  try {
    const datos = await tmdbGet("/discover/movie", {
      "primary_release_date.gte": upcomingReleaseDateStart(),
...
export async function getMovie(idPeli) {
  try {
    ...
    return isAllowedRelease(movie) && pickYoutubeVideo(movie?.videos?.results)
      ? movie
      : {};
  } catch (error) {
...
export async function searchMovies(query) {
  ...
  try {
    ...
    return { ...data, results: filterAllowedReleases(data?.results) };
  } catch (error) {
