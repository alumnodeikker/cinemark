...
export async function traerPeliculasRecientes() {
  try {
...
export async function traerTopSemanal() {
  try {
...
export async function traerProximosEstrenos() {
  try {
    ...
    const conTrailers = await Promise.all(
      lista.slice(0, 12).map(async (peli) => ({
        ...peli,
        trailer_key: await fetchTrailerKey(peli.id),
      }))
    );
...
export async function traerCelebridadesPopulares() {
  try {
...
export async function fetchActor(idActor) {
  try {
